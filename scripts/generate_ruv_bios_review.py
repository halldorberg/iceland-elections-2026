...
def render_card(entry):
    ...
    return (
        f'<article class="card" id="card-{html.escape(rid)}" data-rid="{html.escape(rid)}">'
        f'  <header class="card-header">'
        f'    <div class="card-title-row">'
        f'      <div class="card-title">{ballot}. {name}</div>'
        f'      <button class="approve-btn" data-rid="{html.escape(rid)}" onclick="toggleApprove(this)">Samþykkja</button>'
        f'    </div>'
...
def render():
    ...
    page = (
        ...
        '.approve-btn{flex-shrink:0;background:transparent;color:var(--green);border:1px solid var(--green);border-radius:8px;padding:4px 12px;font-size:12px;font-weight:600;cursor:pointer;transition:background .12s;font-family:inherit;}'
        '.approve-btn:hover{background:rgba(63,185,80,.12);}'
        '.approve-btn.approved{background:var(--green);color:#0d1117;}'
        '.approve-btn.approved::before{content:"✓ ";}'
        '.card.approved{opacity:.55;border-color:rgba(63,185,80,.4);}'
        '.card.approved:hover{opacity:1;}'
        '#topbar{position:sticky;top:0;z-index:100;background:var(--bg);border-bottom:1px solid var(--border);padding:12px 0;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;}'
        '#topbar-left{display:flex;align-items:center;gap:14px;flex-wrap:wrap;}'
        '#count-chip{background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:6px 14px;font-size:13px;color:var(--muted);}'
        '#count-chip strong{color:var(--green);font-size:14px;}'
        '#count-chip .total{color:var(--text);}'
        '#confidence-chip{background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:6px 14px;font-size:13px;color:var(--muted);}'
        '#confidence-chip strong{color:var(--accent);font-size:14px;}'
        '#randomize-btn{background:var(--accent);color:#000;border:none;border-radius:8px;padding:8px 16px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;}'
        '#randomize-btn:hover{opacity:.9;}'
        '#randomize-btn:disabled{opacity:.5;cursor:not-allowed;}'
        '.card.flash{animation:flash 1.4s ease-out;}'
        '@keyframes flash{0%{box-shadow:0 0 0 3px var(--accent),0 0 18px rgba(88,166,255,.5);}100%{box-shadow:none;}}'
        '</style></head><body>'
        ...
        '<div id="topbar">'
        '  <div id="topbar-left">'
        f'    <span id="count-chip">Samþykkt: <strong id="approved-count">0</strong> <span class="total">af {len(data)}</span></span>'
        ...
        'async function unlock(){const v=document.getElementById("pwd").value;const h=await sha1(v);if(h===HASH){document.getElementById("lock-screen").style.display="none";document.getElementById("main").style.display="block";sessionStorage.setItem("ruv_unlocked","1");hydrate();}else{document.getElementById("lock-error").style.display="block";}}'
        ...
        # Approval state in localStorage
        'const STORAGE_KEY="ruv-bios-approved-v1";'
        'function loadApproved(){try{return new Set(JSON.parse(localStorage.getItem(STORAGE_KEY)||"[]"));}catch(e){return new Set();}}'
        'function saveApproved(s){localStorage.setItem(STORAGE_KEY,JSON.stringify([...s]));}'
        ...
        # Confidence given sample n drawn from population N at margin E (default 0.05), worst-case p=0.5
        'function confidenceFromSample(n,N,E){if(n<=0)return 0;if(n>=N)return 1;const n0=n*(N-1)/(N-n);const z=Math.sqrt(n0*E*E/0.25);return erf(z/Math.SQRT2);}'
        'function updateCount(){const total=document.querySelectorAll(".card[data-rid]").length;document.getElementById("approved-count").textContent=APPROVED.size;document.querySelector("#count-chip .total").textContent="af "+total;const conf=confidenceFromSample(APPROVED.size,total,0.05);document.getElementById("confidence-pct").textContent=APPROVED.size===0?"—":(conf*100).toFixed(1)+"%";}'
        'function applyState(card){const rid=card.getAttribute("data-rid");const isApp=APPROVED.has(rid);card.classList.toggle("approved",isApp);const btn=card.querySelector(".approve-btn");if(btn){btn.classList.toggle("approved",isApp);btn.textContent=isApp?"✓ Samþykkt":"Samþykkja";if(isApp)btn.classList.add("approved");}}'
