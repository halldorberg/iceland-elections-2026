...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .new-tag {
      position: absolute; top: max(52px, calc(env(safe-area-inset-top) + 38px)); left: 14px; z-index: 5;
      padding: 3px 10px; border-radius: 999px; background: #f9a825; color: #0f1923;
...
<body>
  ...
  <script>
    ...
    // Returning visitors: videos added since their last visit go FIRST
    // (marked NYTT), then the feed continues from where they left off.
    let FEED = VIDEOFEED, startAtSaved = true;
    const knownRaw = localStorage.getItem('videofeed-known');
    const savedId = localStorage.getItem('videofeed-pos');
    if (knownRaw && savedId) {
      try {
        const known = new Set(JSON.parse(knownRaw));
        const fresh = VIDEOFEED.filter(v => !known.has(v.id)).map(v => ({ ...v, isNew: true }));
        if (fresh.length) {
          const rest = VIDEOFEED.filter(v => known.has(v.id));
          let si = rest.findIndex(v => v.id === savedId);
          if (si < 0) si = 0;
          FEED = [...fresh, ...rest.slice(si), ...rest.slice(0, si)];
          startAtSaved = false;
        }
      } catch (e) {}
    }
    localStorage.setItem('videofeed-known', JSON.stringify(VIDEOFEED.map(v => v.id)));

    feedEl.innerHTML = FEED.map((v, i) => `
      <div class="slide" data-i="${i}">
        ...
        ${v.isNew ? '<div class="new-tag">NÝTT</div>' : ''}
    ...
    const io = new IntersectionObserver(es => {
      es.forEach(e => {
        ...
        if (e.isIntersecting) {
          ...
          if (FEED[i] && !FEED[i].isNew) localStorage.setItem('videofeed-pos', FEED[i].id);
        } else { v.pause(); v.currentTime = 0; }
      });
    }, { threshold: 0.6 });

    // Resume where the visitor left off (unless new videos were prepended —
    // then we start at the top so they see the new ones first).
    if (startAtSaved && savedId) {
      const idx = FEED.findIndex(v => v.id === savedId);
