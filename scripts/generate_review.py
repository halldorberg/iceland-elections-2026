...
function unlock() {
  ...
  if (v === PW) {
    ...
    sessionStorage.setItem('scan-review-unlocked', '1');
    // Pull latest cross-device state once unlocked.
    hydrateFromInbox();
  } else {
...
if (sessionStorage.getItem('scan-review-unlocked') === '1') {
  document.getElementById('lock-screen').style.display = 'none';
  document.getElementById('main').style.display = 'block';
}
...
// ─── Inbox auto-sync (cross-device via ntfy.sh) ────────────────────────
// Each change posts the full state (approvals + comments) to a fixed
// ntfy.sh topic that's unguessable but constant across devices. On page
// load, we pull the latest state back so the user can continue from any
// device without copy/paste. The same topic is used by Claude on the
// other side: `curl -s "<INBOX_URL>/json?poll=1" | tail -1`.
const INBOX_TOPIC = 'lyd-scan-review-0b9f09719c91b7145ea812e418c55d805da16d2f';
const INBOX_URL   = 'https://ntfy.sh/' + INBOX_TOPIC;
let _syncTimer = null;
let _lastSyncOk = null;            // null = unknown, true = ok, false = failed
let _lastSyncedSerialized = null;  // skip POSTs when state is unchanged
let _saveCount = 0;
...
async function _doSync() {
  ...
  if (serialized === _lastSyncedSerialized) {
    // Nothing changed since the last successful POST.
    return;
  }
  _setSyncStatus('pending');
  try {
    const r = await fetch(INBOX_URL, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Title': 'lyd-scan-review-update',
        'Priority': 'min',
        'Tags': 'pencil',
      },
      body: serialized,
    ...
    _lastSyncOk = true;
    _lastSyncedSerialized = serialized;
    _setSyncStatus('ok', 'Synced to inbox · ' + new Date().toLocaleTimeString());
  } catch (err) {
...
// Pull the latest cached message from the ntfy topic and hydrate localStorage
// from it, so the user can pick up where they left off on another device.
async function pullFromInbox() {
  try {
    const r = await fetch(INBOX_URL + '/json?poll=1', { method: 'GET' });
    if (!r.ok) throw new Error('HTTP ' + r.status);
    const text = await r.text();
    const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
    if (!lines.length) return null;
    // Walk lines from newest backward; each line is an ntfy envelope.
    for (let i = lines.length - 1; i >= 0; i--) {
      try {
        const env = JSON.parse(lines[i]);
        if (env.event !== 'message' || !env.message) continue;
        return JSON.parse(env.message);
      } catch (_e) { /* skip malformed */ }
    }
    return null;
  } catch (err) {
    console.warn('pullFromInbox failed:', err);
    return null;
  }
}
...
async function hydrateFromInbox() {
  _setSyncStatus('pending');
  const remote = await pullFromInbox();
  if (!remote) {
    // No remote state yet — keep whatever localStorage has, push it up.
    _setSyncStatus('idle');
    syncToInbox();
    return;
  }
  // Replace local approval/comment storage with the remote snapshot.
  Object.keys(localStorage).filter(k => k.startsWith(APPROVE_KEY_PREFIX) || k.startsWith(COMMENT_KEY_PREFIX))
        .forEach(k => localStorage.removeItem(k));
  (remote.approvals || []).forEach(({id, kind}) => {
    if (!id || APPLIED_IDS.has(id)) return;
    localStorage.setItem(APPROVE_KEY_PREFIX + id, '1');
    localStorage.setItem(APPROVE_KEY_PREFIX + id + ':kind', kind || 'as-is');
  });
  (remote.comments || []).forEach(({cid, text}) => {
    if (cid && text && text.trim()) localStorage.setItem(COMMENT_KEY_PREFIX + cid, text);
  });
  // Re-render UI.
  applyStateToCheckboxes();
  restoreComments();
  refreshCounter();
  // Mark this state as already synced so we don't immediately re-POST it.
  _lastSyncedSerialized = JSON.stringify(_buildInboxPayload());
  _lastSyncOk = true;
  _setSyncStatus('ok', 'Pulled from inbox · ' + new Date(remote.ts || Date.now()).toLocaleTimeString());
}
...
window.addEventListener('load', () => {
  ...
  refreshCounter();
  // If we're already unlocked from a prior session on this browser, pull the
  // latest cross-device state right away. Otherwise wait for unlock().
  if (sessionStorage.getItem('scan-review-unlocked') === '1') {
    hydrateFromInbox();
  }
});
...
<div id="approve-toolbar">
  ...
  <div id="sync-status" data-state="idle" onclick="togglePanel()" style="cursor:pointer" title="Auto-syncs approvals + comments to a private cloud inbox so you can continue from any device. Click to open approval panel."></div>
</div>
