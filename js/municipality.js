...
import { getMunicipalityPartyData } from './data/candidates.js?v=19';
...
function openModal(id) {
  ...
  // Instagram section (lazy — injected only when this modal opens)
  const igSection = document.getElementById('modal-instagram-section');
  const igFeed   = document.getElementById('modal-instagram-feed');
  const igLink   = c.social && c.social.find(s => s.type === 'instagram');
  if (igLink) {
    igSection.style.display = '';
    const handle = igLink.url.replace(/\/$/, '').split('/').pop();
    igFeed.innerHTML = `
      <a class="ig-profile-card" href="${igLink.url}" target="_blank" rel="noopener noreferrer">
        <div class="ig-card-header">
          <div class="ig-icon-wrap">
            <svg class="ig-icon" viewBox="0 0 24 24" fill="currentColor" width="26" height="26" aria-hidden="true">
          ...
          <span class="ig-at">@${handle}</span>
        </div>
        <span class="ig-view-btn">Skoða Instagram ↗</span>
      </a>`;
  } else {
    igSection.style.display = 'none';
    igFeed.innerHTML = '';
  }
